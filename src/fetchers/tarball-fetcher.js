...
export default class TarballFetcher extends BaseFetcher {
  ...
  createExtractor(
    ...
    extractorStream.pipe(untarStream).on("finish", () => {
      ...
      const hexDigest = this.validateIntegrity
        ? this.validateIntegrity.hexDigest()
        : "";
      if (
  ...
  async fetchFromLocal(override: ?string): Promise<FetchedOverride> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const {
        ...
        extractorStream
      } = this.createExtractor(resolve, reject, tarballPath);
  ...
  _supportedIntegrity({
    ...
    if (
      expectedIntegrityAlgorithms.length === 0 &&
