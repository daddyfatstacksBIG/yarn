...
async function fetchCache(
  ...
  const {hash, package : pkg, remote : cacheRemote} =
  ...
  if (remote.integrity) {
    if (!cacheIntegrity ||
        !ssri.parse(cacheIntegrity).match(remote.integrity)) {
      throw new SecurityError(
          config.reporter.lang('fetchBadIntegrityCache', pkg.name,
                               cacheIntegrity, remote.integrity),
      );
    }
  }
  ...
  if (remote.hash) {
    if (!cacheHash || cacheHash !== remote.hash) {
      throw new SecurityError(config.reporter.lang(
          'fetchBadHashCache', pkg.name, cacheHash, remote.hash));
    }
  }
...
export async function fetchOneRemote(
  ...
  if (remote.type === 'link') {
    ...
    return Promise.resolve(
        {resolved : null, hash : '', dest, package : mockPkg, cached : false});
  }
...
function fetchOne(ref: PackageReference,
                  config: Config): Promise<FetchedMetadata> {
  const dest = config.generateModuleCachePath(ref);

  return fetchOneRemote(ref.remote, ref.name, ref.version, dest, config);
}

async function maybeFetchOne(ref: PackageReference, config: Config): Promise <
    ? FetchedMetadata > {
  try {
...
export function fetch(pkgs: Array<Manifest>,
                      config: Config): Promise<Array<Manifest>> {
  ...
  pkgs = pkgs.filter(pkg => {
    ...
    if (otherPkg) {
      config.reporter.warn(
          config.reporter.lang('multiplePackagesCantUnpackInSameDestination',
  ...
  return promise.queue(
      ...
      async pkg => {
        ...
        if (res) {
          ...
          // but only if there was a hash previously as the tarball fetcher does
          // not provide a hash.
          if (ref.remote.hash) {
            ...
            if (ref.remote.hash !== res.hash && config.updateChecksums) {
              ...
              if (ref.remote.resolved) {
                ref.remote.resolved =
                    ref.remote.resolved.replace(oldHash, res.hash);
              }
              ref.config.cache =
                  Object.keys(ref.config.cache).reduce((cache, entry) => {
