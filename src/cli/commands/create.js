...
export function coerceCreatePackageName(str: string): Object {
  ...
  const coercedName =
    pkgNameObj.name !== "" ? `create-${pkgNameObj.name}` : `create`;
  const coercedPkgNameObj = {
    ...pkgNameObj,
    name: coercedName,
    fullName: [pkgNameObj.scope, coercedName].filter(Boolean).join("/"),
    full: [pkgNameObj.scope, coercedName, pkgNameObj.path]
      .filter(Boolean)
      .join("/")
  };
...
export async function run(
  ...
  const { fullName: packageName, name: commandName } = coerceCreatePackageName(
    builderName
  );
