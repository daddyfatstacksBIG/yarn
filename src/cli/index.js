...
export async function main({
  ...
  let isKnownCommand = Object.prototype.hasOwnProperty.call(
    commands,
    commandName
  );
  ...
  if (
    commander.nodeVersionCheck &&
    !semver.satisfies(process.versions.node, constants.SUPPORTED_NODE_VERSIONS)
  ) {
    reporter.warn(
      reporter.lang(
  ...
  const run = (): Promise<void> => {
    ...
    return command
      .run(config, reporter, commander, commander.args)
      .then(exitCode => {
        if (shouldWrapOutput) {
  ...
  folderOptionKeys.forEach(folderOptionKey => {
    ...
    const resolvedFolderOption = folderOption
      ? path.resolve(commander.cwd, folderOption)
      : folderOption;
  ...
  await config
    ...
    .then(() => {
      ...
      if (
        ...
        !fs.existsSync(
          path.join(config.lockfileFolder, constants.LOCKFILE_FILENAME)
        )
      ) {
