...
const {
  execCommand
}: $FlowFixMe = require("../../src/util/execute-lifecycle-script");
...
test("properly handle env command", (): Promise<void> =>
  runRun(["env"], {}, "no-args", (config, reporter): ?Promise<void> => {
    ...
    result[pathVarName] = result[pathVarName]
      ? result[pathVarName].split(path.delimiter)
      : [];
    // $FlowFixMe
    env[pathVarName] = env[pathVarName]
...
test("adds quotes if args have spaces and quotes", (): Promise<void> =>
  runRun(["cat-names", "--filter", '"cat names"'], {}, "bin", config => {
    const script = path.join(config.cwd, "node_modules", ".bin", "cat-names");
    const quotedCatNames =
      process.platform === "win32" ? '^"\\^"cat^ names\\^"^"' : `'"cat names"'`;
...
test("adds workspace root node_modules/.bin to path when in a workspace", (): Promise<void> =>
  runRunInWorkspacePackage(
    ...
    (config, reporter): ?Promise<void> => {
      ...
      const parsedLogData = JSON.parse(
        logEntry ? logEntry.data.toString() : "{}"
      );
      const envPaths = (parsedLogData.PATH || parsedLogData.Path).split(
        path.delimiter
      );
