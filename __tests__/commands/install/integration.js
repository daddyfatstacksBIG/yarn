...
test("install should not copy the .bin folders from the cache", () =>
  runInstall({}, "install-no-bin", async config => {
    expect(await fs.exists(`${config.cwd}/node_modules/is-pnp/.bin`)).toEqual(
      false
    );
  }));
...
test("install should resolve peer dependencies from same subtrees", () =>
  runInstall({}, "peer-dep-same-subtree", async config => {
    ...
    expect(
      JSON.parse(
        await fs.readFile(`${config.cwd}/node_modules//a/package.json`)
      ).version
    ).toEqual("1.1.0");
    expect(
      fs.exists(`${config.cwd}/node_modules/c/node_modules/a`)
    ).resolves.toEqual(false);
  }));

test("install optional subdependencies by default", () =>
  runInstall({}, "install-optional-dependencies", config => {
    expect(fs.exists(`${config.cwd}/node_modules/dep-b`)).resolves.toEqual(
      true
    );
  }));
...
test("installing with --ignore-optional should not install optional subdependencies", () =>
  runInstall(
    ...
    config => {
      ...
      expect(fs.exists(`${config.cwd}/node_modules/dep-c`)).resolves.toEqual(
        true
      );
      expect(fs.exists(`${config.cwd}/node_modules/dep-d`)).resolves.toEqual(
        true
      );
      expect(fs.exists(`${config.cwd}/node_modules/dep-e`)).resolves.toEqual(
        true
      );
    }
  ));
...
test("running install inside a workspace should run the install from the root of the workspace", () =>
  runInstall(
    ...
    async (config, reporter): Promise<void> => {
      ...
      await fs.writeFile(
        `${config.cwd}/workspace/package.json`,
        JSON.stringify(pkgJson)
      );

      const workspaceConfig = await Config.create(
...
test("packages installed through the link protocol should validate all peer dependencies", () =>
  runInstall(
    { checkFiles: true },
    "check-files-should-not-cross-symlinks",
    async config => {
      expect(
        JSON.parse(
          await fs.readFile(`${config.cwd}/node_modules/.yarn-integrity`)
...
test("installing a package with a renamed file should not delete it", () =>
  runInstall(
    ...
    async (config, reporter): Promise<void> => {
      ...
      expect(
        await fs.exists(`${config.cwd}/node_modules/pkg/state.js`)
      ).toEqual(true);
    }
  ));
...
test("properly find and save build artifacts", () =>
  runInstall({}, "artifacts-finds-and-saves", async config => {
    ...
    expect(fs.readFile(path.join(moduleFolder, "dummy.txt"))).resolves.toEqual(
      "foobar"
    );
    expect(
      fs.readFile(path.join(moduleFolder, "dummy", "dummy.txt"))
  ...
  runInstall(
    ...
    async (config, reporter, install, getOutput) => {
      ...
      expect(getOutput()).toContain(
        reporter.lang("implicitFileDeprecated", "bar")
      );
    }
  ));
...
test("don't install with implicit file: protocol if target does not have package.json", () =>
  expect(
    runInstall({ lockfile: false }, "install-file-as-default-no-package")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
      'Couldn\'t find any versions for "foo" that matches "bar"'
    )
...
test("install with explicit file: protocol if target does not have package.json", () =>
  runInstall({}, "install-file-no-package", async config => {
    expect(
      await fs.exists(path.join(config.cwd, "node_modules", "foo", "bar.js"))
    ).toEqual(true);
    expect(
      await fs.exists(path.join(config.cwd, "node_modules", "bar", "bar.js"))
    ).toEqual(true);
  }));
...
test("don't install with file: protocol as default if target is valid semver", () =>
  runInstall({}, "install-file-as-default-no-semver", async config => {
    expect(
      JSON.parse(
        await fs.readFile(
          path.join(config.cwd, "node_modules", "foo", "package.json")
        )
      )
    ).toMatchObject({
      name: "foo"
...
test("install file: dedupe dependencies 1", () =>
  runInstall({}, "install-file-dedupe-dependencies-1", async config => {
    ...
    expect(
      await fs.exists(
        path.join(config.cwd, "node_modules", "a", "node_modules")
      )
    ).toEqual(false);
  }));
...
test("install file: dedupe dependencies 3", () =>
  runInstall(
    ...
    (config, reporter, install, getStdout) => {
      ...
      const messageParts = reporter
        .lang("multiplePackagesCantUnpackInSameDestination")
        .split("undefined");
...
test("install should run install scripts in the order of dependencies", () =>
  runInstall({}, "scripts-order", async (config, reporter) => {
    expect(
      await fs.exists(path.join(config.cwd, "node_modules/dep-a/dep-a-built"))
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, "node_modules/dep-b/dep-b-built"))
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, "node_modules/dep-c/dep-c-built"))
    ).toBe(true);
  }));
      ...
      expect(
        warnings.some(warning => {
          return (
            warning.data
              .toString()
              .indexOf(reporter.lang("invalidResolutionName", "//")) > -1
          );
        })
      ).toEqual(false);
    },
...
test("run install scripts in the order when one dependency does not have install script", () =>
  runInstall(
    ...
    async (config, reporter) => {
      ...
      expect(
        await fs.exists(path.join(config.cwd, "node_modules/dep-d/dep-d-built"))
      ).toBe(true);
    }
  ));
...
test("install should fail to authenticate integrity with incorrect hash and correct sha512", () =>
  expect(
    runInstall({}, "invalid-checksum-good-integrity")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
...
test("install should authenticate integrity field with sha1 checksums", () =>
  runInstall({}, "install-update-auth-sha1", async config => {
    ...
    expect(
      await fs.exists(path.join(config.cwd, "node_modules", "abab"))
    ).toEqual(true);
    expect(
      lockFileLines[3].indexOf("integrity sha1-X6rZwsB/YN12dw9xzwJbYqY8/U4=")
    ).toEqual(2);
  }));
...
test("install should fail to authenticate integrity with correct sha1 and incorrect sha512", () =>
  expect(
    runInstall({}, "install-update-auth-right-sha1-wrong-sha512")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
      "computed integrity doesn't match our records"
    )
...
test("install should fail to authenticate on sha512 integrity mismatch", () =>
  expect(
    runInstall({}, "install-update-auth-wrong-sha512")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
      "computed integrity doesn't match our records"
    )
...
test("install should fail to authenticate on sha1 integrity mismatch", () =>
  expect(
    runInstall({}, "install-update-auth-wrong-sha1")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
      "computed integrity doesn't match our records"
    )
...
test.skip("install should create integrity field if not present", () =>
  runInstall({}, "install-update-auth-no-integrity-field", async config => {
    ...
    expect(
      lockFileLines[2].indexOf("#893312af69b2123def71f57889001671eeb2c853")
    ).toBeGreaterThan(0);
...
test("install should not create the integrity field if missing and auto-add-integrity is false", () =>
  runInstall(
    {},
    "install-update-auth-no-integrity-field-no-auto-add",
    async config => {
      const lockFileContent = await fs.readFile(
        path.join(config.cwd, "yarn.lock")
      );
...
test("install should not create integrity field if not present and in offline mode", () =>
  runInstall(
    { offline: true },
    "install-update-auth-no-offline-integrity",
    async config => {
      expect(
        await fs.exists(path.join(config.cwd, "node_modules", "abab"))
      ).toEqual(true);
...
test("install should ignore existing hash if integrity is present even if it fails to authenticate it", () =>
  expect(
    runInstall({}, "install-update-auth-bad-sha512-good-hash")
  ).rejects.toMatchObject({
    message: expect.stringContaining(
      "computed integrity doesn't match our records"
    )
...
test("install a scoped module from authed private registry", () =>
  runInstall({}, "install-from-authed-private-registry", async config => {
    ...
    expect(
      (await fs.readFile(
        path.join(config.cwd, "node_modules", "@types", "lodash", "index.d.ts")
      )).split("\n")[0]
...
test("install a module with optional dependency should skip incompatible transient dependency", () =>
  runInstall(
    ...
    async config => {
      expect(
        await fs.exists(path.join(config.cwd, "node_modules", "dep-optional"))
      ).toEqual(true);
      expect(
        await fs.exists(
          path.join(config.cwd, "node_modules", "dep-incompatible")
...
if (process.platform !== "darwin") {
  test("install incompatible optional dependency should still install shared child dependencies", () =>
    runInstall(
      ...
      async config => {
        expect(
          await fs.exists(path.join(config.cwd, "node_modules", "deep-extend"))
        ).toEqual(true);
        expect(
...
test("optional dependency that fails to build should not be installed", () =>
  runInstall({}, "should-not-install-failing-optional-deps", async config => {
    expect(
      await fs.exists(path.join(config.cwd, "node_modules", "optional-failing"))
    ).toEqual(false);
  }));
...
test("failing dependency of optional dependency should not be installed", () =>
  runInstall(
    ...
    async config => {
      expect(
        await fs.exists(path.join(config.cwd, "node_modules", "optional-dep"))
      ).toEqual(true);
      expect(
...
test("transitive file: dependencies should work", () =>
  runInstall({}, "transitive-file", async (config, reporter) => {
    expect(await fs.exists(path.join(config.cwd, "node_modules", "b"))).toBe(
      true
    );
  }));
...
test("unbound transitive dependencies should not conflict with top level dependency", () =>
  runInstall({ flat: true }, "install-conflicts", async config => {
    expect(
      (await fs.readJson(
        path.join(config.cwd, "node_modules", "left-pad", "package.json")
      )).version
    ).toEqual("1.0.0");
  }));
...
test("manifest optimization respects versions with alternation", () =>
  runInstall(
    { flat: true },
    "optimize-version-with-alternation",
    async config => {
        ...
        async (linkConfig): Promise<void> => {
          ...
          await runLink(
            ...
            async (): Promise<void> => {
              // check that it exists (just in case)
              const existed = await fs.exists(
                path.join(installConfig.cwd, "node_modules", "a-package")
              );
              expect(existed).toEqual(true);
...
test("file: dependency ending with `.git` should work", () =>
  runInstall({}, "local-named-git", async (config, reporter) => {
    expect(await fs.exists(path.join(config.cwd, "node_modules", "a"))).toBe(
      true
    );
  }));
...
test("does not check node_modules for extraneous files when --modules-folder used", async () => {
  ...
  await runInstall(
    { modulesFolder: "./some_modules" },
    "extraneous-node-modules",
    async (config): Promise<void> => {
      ...
      expect(await fs.exists(`${config.cwd}/node_modules/extra.js`)).toEqual(
        true
      );
      // Extraneous files in some_modules should have been cleaned.
      expect(await fs.exists(`${config.cwd}/some_modules/extra.js`)).toEqual(
        false
      );
    }
  );
...
describe("Cache", () => {
  ...
  test("install should fail when cached package integrity does not match lockfile integrity field", () =>
    expect(
      runInstall({}, "install-update-auth-invalid-cache-integrity")
    ).rejects.toThrow(
      // eslint-disable-next-line max-len
  ...
  test("install should fail when cached package hash does not match remote hash", () =>
    expect(
      runInstall({}, "install-update-auth-invalid-cache-hash")
    ).rejects.toThrow(
      // eslint-disable-next-line max-len
