...
addTest("https://github.com/bestander/chrome-app-livereload.git"); // no
// package.json
...
async function runYarn(
  ...
  const { stdout, stderr } = await execa.shell(
    sh`${path.resolve(__dirname, "../bin/yarn")} ${args}`,
    options
  );
...
describe("production", () => {
  test("it should be true when NODE_ENV=production", async () => {
    ...
    expect(JSON.parse(stdoutOutput.toString())).toHaveProperty(
      "production",
      true
    );
  });
  ...
  test("it should prefer CLI over NODE_ENV", async () => {
    ...
    const [stdoutOutput, _] = await runYarn(
      ["--prod", "false", "config", "current"],
      options
    );
  ...
  test("it should prefer YARN_PRODUCTION over NODE_ENV", async () => {
    ...
    const options = {
      cwd,
      env: { YARN_SILENT: 1, YARN_PRODUCTION: "false", NODE_ENV: "production" }
    };
...
test("--mutex network with busy port", async () => {
  ...
  expect(mutexError.message).toMatch(
    new RegExp(en.mutexPortBusy.replace(/\$\d/g, "\\d+"))
  );
});
...
describe("--registry option", () => {
  test("--registry option with npm registry", async () => {
    ...
    const lockfile = explodeLockfile(
      await fs.readFile(path.join(cwd, "yarn.lock"))
    );
...
test("default rc", async (): Promise<void> => {
  const [stdoutOutput] = await runYarn(["cache", "dir"], {
    cwd: customCacheCwd
  });
...
test("--use-yarnrc", async (): Promise<void> => {
  const [stdoutOutput] = await runYarn(
    ["cache", "dir", "--use-yarnrc", "./custom-yarnrc"],
    { cwd: customCacheCwd }
  );
...
test("yarnrc arguments", async () => {
  ...
  expect(
    JSON.parse(await fs.readFile(`${cwd}/package.json`)).dependencies[
      "left-pad"
    ]
  ).toMatch(/^\d+\./);
...
for (const withDoubleDash of [false, true]) {
  test(`yarn run <script> ${withDoubleDash ? "-- " : ""}--opt`, async () => {
    ...
    (exp => (withDoubleDash ? exp : exp.not))(
...
test("cache folder fallback", async () => {
  ...
  expect(stdoutOutput.toString().trim()).toEqual(
    path.join(cacheFolder, `v${constants.CACHE_VERSION}`)
  );
  expect(stderrOutput.toString()).not.toMatch(
    /Skipping preferred cache folder/
  );
  ...
  expect(stdoutOutput2.toString().trim()).toEqual(
    path.join(
      constants.PREFERRED_MODULE_CACHE_DIRECTORIES[0],
...
test("relative cache folder", async () => {
  ...
  const [stdoutOutput, _] = await runYarn(["cache", "dir"], {
    cwd: `${base}/sub`
  });

  // The dirname is to remove the "v2" part
  expect(await fs.realpath(path.dirname(stdoutOutput.toString()))).toEqual(
...
test("yarn init -y", async () => {
  ...
  const initialManifestFile = JSON.stringify({
    name: "test",
    license: "ISC",
    version: "1.0.0"
  });
...
test("--modules-folder option", async () => {
  ...
  // folder
  const importantFile = await fs.readFile(
    `${projectFolder}/IMPORTANT_FILE.txt`
  );
